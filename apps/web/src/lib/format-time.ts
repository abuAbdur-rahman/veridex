...
export function formatRelativeTime(value: string, currentTime = Date.now()): string {
	...
	if (absoluteSeconds < 3_600)
		return relativeTime.format(direction * Math.max(1, Math.round(absoluteSeconds / 60)), "minute");
	if (absoluteSeconds < 86_400)
		return relativeTime.format(direction * Math.round(absoluteSeconds / 3_600), "hour");
	if (absoluteSeconds < 604_800)
		return relativeTime.format(direction * Math.round(absoluteSeconds / 86_400), "day");
