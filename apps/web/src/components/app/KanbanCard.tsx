	...
	return (
		<article
			...
			className={cn(
				...
				active && "border-l-[3px] border-l-[var(--accent)] shadow-[0_8px_24px_rgba(0,0,0,0.35)]",
				!active && "hover:border-[var(--line)] hover:border-l-[3px] hover:border-l-[var(--accent)]",
				className,
			)}
		>
			<div className="flex items-start gap-2">
				<button
					type="button"
					onClick={onOpen}
					aria-label={`Open ${issue.ticketRef}: ${issue.title}`}
				...
				{onMove && getAllowedTransitions(issue.status).length > 0 ? (
					<DropdownMenu>
						<DropdownMenuTrigger
							...
							className="inline-flex min-h-8 items-center gap-1 rounded-md border border-[var(--line)] px-2 text-xs font-medium hover:border-[var(--accent)]"
						>
