	...
	return (
		<div className="mx-auto flex w-full max-w-[840px] flex-col gap-8">
			...
			<section aria-labelledby="invite-member-heading" className="flex flex-col gap-3">
				<SectionLabel>
					<MailPlus className="size-3.5" aria-hidden="true" /> Invite member
				</SectionLabel>
				<form
					className="flex flex-col gap-3 rounded-[10px] border border-[var(--line)] bg-[var(--surface)] p-4 sm:flex-row sm:items-end"
					onSubmit={(event) => void submitInvite(event)}
				>
					<label
						className="flex min-w-0 flex-1 flex-col gap-1.5 text-[13px] font-medium text-[var(--ink)]"
					...
					>
						...
						<input
							...
							className="rounded-lg border border-[var(--line)] bg-[var(--bg)] px-3.5 py-2.5 text-sm text-[var(--ink)] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent-ring)]"
						/>
					</label>
					<button
						type="submit"
						disabled={inviteBusy}
						className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg bg-[var(--accent)] px-4 text-sm font-semibold text-white hover:bg-[var(--accent-strong)] disabled:cursor-wait disabled:opacity-60"
					>
			...
			{inviteLink ? (
				<section aria-label="Shareable invite link">
					...
					<div className="mt-3 flex items-stretch">
						<div className="min-w-0 flex-1 truncate rounded-l-lg border border-r-0 border-[var(--line)] bg-[var(--bg)] px-3 py-2.5 font-[var(--mono)] text-xs text-[var(--ink)]">
							{inviteLink}
						</div>
						<button
							type="button"
							...
							className="inline-flex min-w-11 cursor-pointer items-center justify-center rounded-r-lg border border-[var(--line)] bg-[var(--surface)] text-[var(--ink-soft)] hover:bg-[var(--bg-alt)] hover:text-[var(--ink)]"
						>
							{copied ? (
			...
			<section aria-label="Members">
				...
				{membersPending ? (
					<p className="mt-3 rounded-[10px] border border-[var(--line)] bg-[var(--surface)] px-4 py-8 text-center text-[13px] text-[var(--ink-soft)]">
						Loading members...
					</p>
				) : null}
				{membersError ? (
					...
					>
						{membersError instanceof ApiError ? membersError.message : "Could not load members."}
					</p>
				) : null}
						...
						{members.map((member) => (
							...
							>
								<span className="min-w-0 flex-1">
									<span className="block text-sm font-medium text-[var(--ink)]">{member.name}</span>
									<span className="block truncate font-[var(--mono)] text-xs text-[var(--ink-soft)]">
										{member.email}
									</span>
								</span>
								<span className="rounded-[6px] bg-[var(--bg-alt)] px-2.5 py-1 font-[var(--mono)] text-xs uppercase text-[var(--ink-soft)]">
									{member.teamRole}
								</span>
							</li>
						))}
