...
interface CreateProjectModalProps {
	...
	error?: string;
	onClose?: () => void;
	onSubmit?: (values: CreateProjectValues) => void | Promise<void>;
}
...
export function CreateProjectModal({
	...
	function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		const description =
			String(new FormData(event.currentTarget).get("description") ?? "").trim() || undefined;
		void onSubmit?.({ name: name.trim(), slug, description });
	}
			...
			<DialogContent className="w-full max-w-[560px] gap-0 overflow-hidden rounded-[12px] border border-[var(--line)] bg-[var(--surface)] p-0 text-[var(--ink)] sm:max-w-[560px]">
				<DialogHeader className="border-b border-[var(--line)] px-6 py-4 pr-14">
					...
					<DialogDescription className="sr-only">
						Create a new project in this team.
					</DialogDescription>
				</DialogHeader>
				<form className="flex flex-col gap-5 p-6" onSubmit={handleSubmit}>
					...
					<FormField label="Name" htmlFor="create-project-name" required>
						<input
					...
					<FormField label="Description" htmlFor="create-project-description" hint="Optional.">
						<textarea
